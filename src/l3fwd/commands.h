...
#ifndef _L3FWD_COMMANDS_H_
#define _L3FWD_COMMANDS_H_

#include "shared/secondary/add_port.h"

#define RTE_LOGTYPE_SPP_L3FWD RTE_LOGTYPE_USER1
...
do_del(char *res_uid)
{
	...
	if (!strcmp(p_type, "vhost")) {
		...
		RTE_LOG(DEBUG, SPP_L3FWD, "Del ring id %d\n", p_id);
...
do_connection(int *connected, int *sock)
{
	...
	if (*connected == 0) {
		if (*sock < 0) {
			RTE_LOG(INFO, SPP_L3FWD, "Creating socket...\n");
		...
		RTE_LOG(INFO, SPP_L3FWD, "Trying to connect ... socket %d\n", *sock);
		ret = connect(*sock, (struct sockaddr *) &servaddr,
				sizeof(servaddr));
		if (ret < 0) {
			RTE_LOG(ERR, SPP_L3FWD, "Connection Error");
			return ret;
		}

		RTE_LOG(INFO, SPP_L3FWD, "Connected\n");
		*connected = 1;
	}
...
parse_command(char *str)
{
	...
	while (token_list[max_token] != NULL) {
		RTE_LOG(DEBUG, SPP_L3FWD, "token %d = %s\n", max_token,
	...
	if (!strcmp(token_list[0], "status")) {
		RTE_LOG(DEBUG, SPP_L3FWD, "status\n");
		...
		RTE_LOG(DEBUG, SPP_L3FWD, "exit\n");
		RTE_LOG(DEBUG, SPP_L3FWD, "stop\n");
		cmd = STOP;
		ret = -1;
		...
		RTE_LOG(DEBUG, SPP_L3FWD, "forward\n");
		cmd = FORWARD;

	} else if (!strcmp(token_list[0], "add")) {
		RTE_LOG(DEBUG, SPP_L3FWD, "Received add command\n");
		if (do_add(token_list[1]) < 0)
			RTE_LOG(ERR, SPP_L3FWD, "Failed to do_add()\n");

	} else if (!strcmp(token_list[0], "patch")) {
		RTE_LOG(DEBUG, SPP_L3FWD, "patch\n");
		...
		if (strncmp(token_list[1], "reset", 5) == 0) {
			...
			if (add_patch(in_port, out_port) == 0)
				RTE_LOG(INFO, SPP_L3FWD,
					"Patched '%s:%d' and '%s:%d'\n",
					in_p_type, in_p_id,
					out_p_type, out_p_id);

			else
				RTE_LOG(ERR, SPP_L3FWD, "Failed to patch\n");
			ret = 0;
		}

	} else if (!strcmp(token_list[0], "del")) {
		RTE_LOG(DEBUG, SPP_L3FWD, "Received del command\n");

		cmd = STOP;

		if (do_del(token_list[1]) < 0)
			RTE_LOG(ERR, SPP_L3FWD, "Failed to do_del()\n");
	}
...
do_receive(int *connected, int *sock, char *str)
{
	...
	if (ret <= 0) {
		RTE_LOG(DEBUG, SPP_L3FWD, "Receive count: %d\n", ret);
		if (ret < 0)
			RTE_LOG(ERR, SPP_L3FWD, "Receive Fail");
		else
			RTE_LOG(INFO, SPP_L3FWD, "Receive 0\n");

		RTE_LOG(INFO, SPP_L3FWD, "Assume Server closed connection\n");
		close(*sock);
		*sock = SOCK_RESET;
		*connected = 0;
		return -1;
...
do_send(int *connected, int *sock, char *str)
{
	...
	if (ret == -1) {
		RTE_LOG(ERR, SPP_L3FWD, "send failed");
		*connected = 0;
		return -1;
	}

	RTE_LOG(INFO, SPP_L3FWD, "To Server: %s\n", str);

	return 0;
}

#endif // _L3FWD_COMMANDS_H_
